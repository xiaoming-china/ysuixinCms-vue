...
function content_5a603a5449a842_56100592 (Smarty_Internal_Template $_smarty_tpl) {
echo '<?php
';?>/* @var $this \yii\web\View */
/* @var $exception \Exception */
/* @var $handler \yii\web\ErrorHandler */
<?php echo '?>';
echo '<?php ';?>if (method_exists($this, 'beginPage')) $this->beginPage(); <?php echo '?>';?>
<!doctype html>
<html lang="en">
...
<body>
    <div class="header">
        <?php echo '<?php ';?>if ($exception instanceof \yii\base\ErrorException): <?php echo '?>';?>
            ...
            <h1>
                <span><?php echo '<?=';?> $handler->htmlEncode($exception->getName()) <?php echo '?>';?></span>
                &ndash; <?php echo '<?=';?> $handler->addTypeLinks(get_class($exception)) <?php echo '?>';?>
            </h1>
        ...
        <h2><?php echo '<?=';?> nl2br($handler->htmlEncode($exception->getMessage())) <?php echo '?>';?></h2>

        <?php echo '<?php ';?>if ($exception instanceof \yii\db\Exception && !empty($exception->errorInfo)) {
            echo '<pre>Error Info: ' . print_r($exception->errorInfo, true) . '</pre>';
        } <?php echo '?>';?>

        <?php echo '<?=';?> $handler->renderPreviousExceptions($exception) <?php echo '?>';?>
    </div>

    <div class="call-stack">
        <?php echo '<?=';?> $handler->renderCallStack($exception) <?php echo '?>';?>
    </div>

    <div class="request">
        <div class="code">
            <?php echo '<?=';?> $handler->renderRequest() <?php echo '?>';?>
        </div>
    </div>
...
window.onload = function() {
    ...
    <?php echo '</script'; ?>
>
    <?php echo '<?php ';?>if (method_exists($this, 'endBody')) $this->endBody(); // to allow injecting code into body (mostly by Yii Debug Toolbar) <?php echo '?>';?>
</body>

</html>
<?php echo '<?php ';?>if (method_exists($this, 'endPage')) $this->endPage(); <?php echo '?>';
}
}
