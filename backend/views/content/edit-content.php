                ...
                v-for="(item, index) in modelFieldList">
                    <span slot="label">
                      ...
                      <div v-if="item.type == 'pic_upload'">
                        ...
                        <div :id="'upload_'+item.e_name"  class="ivu-upload" style="display: inline-block; width: 58px;" @click="openFile(item.e_name);" v-if="item.seetings.many_select === 'false' && item.value == ''">
...
<script>
    new Vue({
        ...
        mounted: function() {
            ...
            this.contentInfo.id           = this.request('id');
        ...
        methods: {
          ...
          getModelField:function(){
            var _that = this;
            $ajax(
              '/admin/content/get-content-field',
              {
              ...
              'get',
              function(res){
                _that.modelFieldList = res.data.model_field.list;
                //加载所有是富文本的字段编辑器
                _that.$nextTick(function () {
                    var length = res.data.model_field.list.length;
                    var data   = res.data.model_field.list;
                    _that.contentInfo.status        = res.data.model_field.info.status;
                    _that.contentInfo.publish_time  = res.data.model_field.info.publish_time;
                    _that.contentInfo.allow_comment = res.data.model_field.info.allow_comment;
                    _that.contentInfo.show_template = res.data.model_field.info.show_template;
                    for (var i = 0; i < length; i++) {
